...
- **v0.9.13 Security hardening**: Process security & lifecycle hardening — Rust zeroize/secrecy (cert_bootstrap, reveal-secret stdout), C# SecretString (zeroed on scope exit), named pipe DACL (BA/SY/OW only), binary hash self-verification + debugger detection + WER/crash-dump disable + DLL injection enum + VirtualLock (service), NTFS ACL on audit files, AES-256-GCM at-rest encryption for audit.json (EncryptAuditContent/DecryptAuditContent wired into read/write), export-state double-layer crypto (AES-GCM payload + DPAPI-wrapped DEK + HMAC-SHA256, v1 backward compat), TOCTOU mutex in reconcile loop, provider binary hash verification (sops/op SHA256 on first use)
...
- **v0.9.14 InputDialog + Sticky notes + Material 3 tabs**: `InputDialog.svelte` replaces `window.prompt()` globally (rename, add-var/profile-input). Variable sticky-note annotations stored in `var-notes.json`. Material 3 ARIA tablist with roving tabindex. PATH scope-aware apply/unapply fix (root cause of PATH entries not applied when profile enabled). Compile-before-package ordering enforced — `build.mjs` must not be invoked with skip flags after source changes.
- **v0.9.16 ProfileShow source-aware + path scope badge + ServicePage i18n**: `profile show` output now includes the source profile name for each variable/path. ProfilePage renders a "from {profile}" badge. ServicePage fully localized across all 10 locales. `mount_id` IPC parameter aligned to snake_case across Rust/C#.
- **v0.9.17 PathEditor reverse index + ProfilePage pathScopes badge**: `PathEditor.svelte` uses reverse index lookup for move operations. ProfilePage displays per-entry scope badges (user/system) for PATH entries. IPC `mount_id` field fully snake_case aligned.
- **v0.9.18 Service rotate + WebView2 hardening**: `call_cli_rotate()` in `reconcile.rs` sets `CREATE_NO_WINDOW` flag to prevent console flash. `ServicePage.svelte` formats timestamps via `toLocaleString()`. WebView2 native right-context menu fully suppressed via `oncontextmenu preventDefault` + CSS `user-select: none` (clash-verge-rev pattern).
- **v0.9.19 SWR caching + preloading**: All 5 read APIs (listVariablesRaw, listPathEntries, listHistory, listProtection, listProfiles) use stale-while-revalidate pattern with 15s TTL. `listProfiles` cache added (previously un-cached, largest latency source). ProtectionPage 4-way concurrent IPC. Tab-adjacent page preloading on startup.
- **v0.9.20 UI Design System — dual-axis theme tokens**: 6-color theme system (slate/blue/violet/rose/cyan/amber) with `data-theme` + `data-theme-style` attribute selectors. All `dark:` Tailwind prefixes removed; dark mode driven by `[data-theme="dark"]`. All inline SVGs replaced with `lucide-svelte`. Scoped CSS transition (`.theme-changing` class). WebView2 right-click context menu suppression. Tab indicator width 0px initial state fix.
- **v0.9.20 Lightweight mode + tray fix**: `CheckMenuItem.set_enabled(true)` after `set_text` to fix muda HMENU rebuild bug. `close` = minimize-to-tray; quit only via tray. Checkmark sync on every `update_lightweight_check`.
