...
- `src/*.cs` -- C# .NET 10 CLI backend (env-manager-cli.exe); `src/Program.cs` is the thin Main dispatcher, `src/CliRuntime.cs` holds the shared CLI runtime, one module file per command domain
