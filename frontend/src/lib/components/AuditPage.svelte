...
<div class="space-y-3">
  <div class="flex items-center justify-between">
    <div>
      <h2 class="text-base font-semibold text-foreground">{$t('nav.audit')}</h2>
      <p class="text-xs text-muted-foreground mt-1">{$t('audit.description')}</p>
    </div>
    <button
      on:click={loadAudit}
  ...
  {:else if auditEntries.length > 0}
    ...
    <p class="text-xs text-muted-foreground">{auditEntries.length} {$t('audit.entries')}</p>
  {:else}
