<script lang="ts">
  ...
  async function handleRemove(path: string) {
    if (!confirm($t('path.confirmRemove'))) return
    actionLoading = true
    try {
    ...
    <div class="flex-1 overflow-y-auto px-5 py-4">
      ...
      <div class="flex gap-2 mb-3">
        <input
          ...
          class="flex-1 px-3 py-1.5 text-xs border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 font-mono dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
        />
        <button
          on:click={handleAdd}
