...
+- scripts/                    # Build orchestrator (build.mjs), test harness, rsvg-convert wrapper, migration scripts, snapshot scripts; cognitive-complexity guard + per-module Stryker scorer + Tauri updater latest.json generator (ticket 44) with node:test pin
...
Secret provider contract tests (architecture-recovery issue 10) pin all eight `ISecretProvider` implementations to one shared behavior contract: an abstract `SecretProviderContractTests` base (fail-closed decryption, round-trip, stable malformed-format error, plaintext-never-in-the-envelope — each expressed only through the `ISecretProviderHarness` seam), one sealed mount per provider, and a `SecretProviderContractComplianceTests` reflection gate that fails the build when an implementation lacks a mount. `DpapiCurrentUserContractTests` runs its backend-dependent assertions on the real local DPAPI backend (L0); the other seven mounts run the backend-independent assertions (fail-closed, malformed-format) and `Skip` the backend-dependent round-trip/plaintext assertions with the layer reason (L1/L2, see docs/architecture.md "Secret Provider Contract Test Suite"). The L1 `Skip` surface is not an uncovered path: the `verify-l1` CI job (ubuntu-latest, Docker preinstalled) runs `--filter "Category=L1"` with `EM_L1_MATRIX=1` on every PR/push/dispatch, so the locally-skipped assertions execute in CI.
...
Profile aggregate-root encapsulation (architecture-recovery ticket 42, spec Phase 6 story 22, mental-model aggregate root): `ProfileData` (`src/Models.cs`) becomes a true aggregate root - all property setters private, each property carrying `[JsonInclude]` so System.Text.Json keeps round-tripping profiles.json through the private accessors. State changes go exclusively through domain methods: `AddVariable`/`RemoveVariable` (case-insensitive name-collision semantics), `AddPathEntry`/`RemovePathEntry` which own the `NormalizePathEntry` invariant and keep `PathScopes` index-aligned with `PathEntries`, `AddSecretVariable`/`RemoveSecretVariable`, and `SetLaunchTarget`/`SetLaunchArguments`/`SetWorkingDirectory`/`SetName`/`SetEnabled`/`SetAppliedAt`/`SetProfileType`/`SetSchemaVersion`/`SetInherits`/`SetPathEntries`/`SetPathScopes`/`SetSecretVariables`/`SetResolvedPaths`. Call sites migrated in `ProfileCommand.cs`/`ProfileStorage.cs`/`ProfileLaunchCommand.cs`/`ProfileSecretCommand.cs` (object initializers and bare property writes replaced by domain-method calls); `SchemaMigration.cs`/`Audit/Recording/ProfileAudit.cs` switched to `EnsureCollections()`/`SetSchemaVersion()`. SecretMount remains a separate aggregate - `ProfileData` holds only secret NAMES and `mount:<id>` value references, never `SecretMount` objects (delta #3 verified-invariant, no schema change). Behavior-preserving refactor: the 62 ProfileCommand characterization snapshots, the seam tests, ProgramMethodBudgetTests (201 - no new Program-partial methods), and the cognitive-complexity gate (domain methods all below threshold 15) are unaffected; test fixtures (MkProfile, MutationSurvivorTriageTests, CliOutputSnapshotTests, ProfileSeamValidationTests) use the domain-method construction form.

Auto-update wiring onto tauri-plugin-updater (architecture-recovery ticket 37, spec Phase 6 story 17, mental-model hexagonal edge at the updater boundary): `frontend/src-tauri/Cargo.toml` adds `tauri-plugin-updater = "2.11.0"` and `frontend/package.json` adds `@tauri-apps/plugin-updater ^2.11.0` (lockfiles committed). `main.rs` registers `tauri_plugin_updater::Builder::new().build()` and adds two commands: `check_update_plugin` (calls `app.updater().check()` against the `latest.json` endpoint + minisign pubkey already declared in `tauri.conf.json`; returns the same UpdateInfo JSON shape as the legacy PowerShell `check_for_updates`, which stays in `main.rs` as the browser-fallback transport) and `download_and_install` (stateless re-check + signature-verified MSI install; the app exits itself once the installer launches on Windows). `capabilities/default.json` gains `"updater:default"`. SettingsDialog's download button invokes `download_and_install` (install failure falls back to opening the release page) and onMount runs one silent background check that only sets the update badge - never toasts. The frontend consumes the plugin through `checkUpdatePlugin()` / `downloadAndInstall()` wrappers in `api.ts`. i18n x10 adds `update.downloading` / `update.installPrompt` / `update.checkFailed` (flat keys after `update.error`); the ticket-36 anchors (`await appVersion()` in SettingsDialog, `app_version` in the handler list, no hardcoded version literal) are preserved and still pinned by `review-regressions.test.ts`.

Release workflow + latest.json + SLSA L2 provenance (architecture-recovery ticket 44, spec Phase 6 Round 7, HARD GATE - user authorization on 2026-09-09): closes the updater loop end-to-end in CI without the window ever calling `gh secret set`. The package job's new "Bundle and sign MSI for Tauri updater" step zips `release/msi/*.msi` to `<basename>.msi.zip` and runs `npx --no-install tauri signer sign` with the existing `TAURI_SIGNING_PRIVATE_KEY` / `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` GitHub Secrets (both predate this ticket; verification in `reports/44`); the `env-manager-msi` upload path now globs `release/msi/*.msi` + `release/msi/*.msi.zip` + `release/msi/*.msi.zip.sig` so the artifact set carries the updater triple. The release job declares the same secrets at the job-level `env:` block, runs the new `Generate latest.json (ticket 44)` step (`node scripts/gen-latest-json.mjs --assets-dir artifacts --version $version --url-base <GitHub release download URL> --out artifacts/latest.json`), and uploads `artifacts/*` (latest.json included) via softprops/action-gh-release. The `actions/attest-build-provenance` step (SLSA L2 build provenance) is re-pinned from `97770d5af...` v4.1.0 - which is no longer reachable upstream, the same force-push failure recorded in WORKFLOW.md §6 ticket-30 lesson log - to v4.2.2 = `4d101475d8b20a2381f78447822ac1eab6504dd8` (verified 2026-09-09 via `gh api repos/actions/attest-build-provenance/commits/<sha>`). `scripts/gen-latest-json.mjs` is the new zero-dependency Node ESM helper; it emits the manifest shape `{version, notes, pub_date, platforms.windows-x86_64.{url,signature}}` that `@tauri-apps/plugin-updater` v2 expects, refuses to fabricate a `signature` (missing `.sig` is fail-closed: the script exits 1 so the release job cannot ship an updater manifest pointing at unsigned bytes), and defaults `pub_date` to current UTC RFC3339. Schema-pinned by `scripts/gen-latest-json.test.mjs` (`node --test`, runs in the build.yml verify job as step "Unit tests for scripts/gen-latest-json.mjs").
```bash
