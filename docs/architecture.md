...
**GUI**: The Settings dialog has a "Check for Updates" button. Clicking it first invokes the `app_version` Tauri command (returns the Cargo package version, which release tooling keeps in sync with the csproj `<Version>` single source, ADR 0003), then invokes `check_update_plugin` - the GUI never hardcodes a version literal (pinned by `review-regressions.test.ts`). Ticket 37 wires the flow onto **tauri-plugin-updater**: `check_update_plugin` in `main.rs` calls `app.updater().check()` against the `latest.json` endpoint declared in `tauri.conf.json` (`plugins.updater`, active since v0.7 with pubkey + endpoint already configured), verifies the minisign signature, and returns the same `UpdateInfo` JSON shape the legacy `check_for_updates` (PowerShell GitHub-API probe, kept as a browser fallback) produced, so the frontend contract is unchanged. When an update is found the download button invokes `download_and_install`, which re-checks (stateless command, no pending-update State plumbing), downloads the signature-verified MSI, and launches the installer - on Windows the app exits itself once the installer starts. A silent background check runs once when the Settings dialog opens (badge only: it sets the update-available state, never toasts); manual checks and install failures surface toasts, and install failure falls back to opening the release page in the browser. The check remains user-triggered on dialog open - no periodic background polling. The release-side half of the chain (ticket 44) closes it: the `build.yml` package job zips the MSI and signs it with the existing `TAURI_SIGNING_PRIVATE_KEY`/`_PASSWORD` GitHub Secrets (`npx --no-install tauri signer sign`), the `env-manager-msi` upload carries the `.msi`/`.msi.zip`/`.msi.zip.sig` triple, the release job runs `scripts/gen-latest-json.mjs` to assemble `latest.json` against the artifact set (version / notes / `pub_date` RFC3339 / `platforms.windows-x86_64.{url,signature}`), and `actions/attest-build-provenance@v4.2.2` writes the SLSA L2 provenance for every `artifacts/*` (including the new updater manifest and signature). The generator never fabricates a signature: a missing `.sig` (unsigned package job run, e.g. a fork without the secrets) is fail-closed - the release step exits non-zero rather than publishing a manifest pointing at unsigned bytes. The schema is pinned by `scripts/gen-latest-json.test.mjs` (`node --test`, build.yml verify job).
...
In this first round only DPAPI runs its backend-dependent assertions on a real backend; the other seven mounts run the backend-independent assertions (fail-closed, malformed-format) and `Skip` the backend-dependent ones with the layer reason. The L1 `Skip` surface is exercised in CI by the `verify-l1` job (ubuntu-latest + Docker, `EM_L1_MATRIX=1`, `--filter "Category=L1"` - see "Secret Provider L1 Emulator Matrix" below), so the locally-skipped assertions are covered on every PR/push/dispatch.
...
| # | Paradigm | Target (what it catches / locks) | Tool | CI tier |
|---|----------|---------------------------------|------|---------|
| 1 | Pure-function unit test | Argument tokenizer (`LenientArgs.Tokenize`), exception scrubbing (`ScrubExceptionMessage`), PATH entry normalization (`NormalizePathEntry`) | xUnit (`dotnet test`) | Tier 1 -- every PR (parallel) |
| 2 | Engine-seam behavior test | `WritePathSeamTests`: set/delete/toggle/rename/change-scope/PATH-list command cores against `InMemoryScope` with synthetic protection predicates; protected-entry rejection, write-verify-delete order, scope selection, broadcast timing | xUnit + `IEnvironmentScope` seam | Tier 1 -- every PR (parallel) |
| 3 | Profile/secret seam migration test | `ProfileSeamValidationTests`: v0.7.7 inheritance-chain secret-propagation gate via `RunProfilePreflight`, hermetic per-test `profiles.json` redirect, falsifiable launch-inherits-secret-launch poisoned-JSON variant | xUnit + seam redirect | Tier 1 -- every PR (serial collection) |
| 4 | Canary redaction (pure-function core) | `CanaryRedactionTests` + `ScrubExceptionMessageTests`: format-shaped canaries (password=/Bearer/VAULT_TOKEN=) never survive `ScrubExceptionMessage`; `<redacted>` placeholder appears; un-patterned values pass through | xUnit | Tier 1 -- every PR (parallel) |
| 5 | Canary redaction (live sink scan) | `tests/canary-redaction.Tests.ps1`: 7-sink zero-leak scan (profile show/preview/list, history audit, launch stdout, error stderr) + positive child-env control + masking-placeholder positive assertions | Pester | Tier 3 -- every PR (windows-latest verify job) |
| 6 | Pester launch injection golden | `tests/launch-env-injection.Tests.ps1`: golden env diff, probe-process echo, Launch-never-writes-registry invariant (injected names absent from `HKCU\\Environment`) | Pester | Tier 3 -- every PR (windows-latest) |
| 7 | Test-residue hygiene | `scripts/test-with-restore.ps1` reconciliation block + `scripts/check-test-residue.ps1`: harness-registered `EM_TEST_*` only; foreign-drift fails run; self-check exit 0 = clean, 1 = residue | PowerShell + harness redirect seams | Tier 3 -- every PR (windows-latest) |
| 8 | CI user-state isolation | `LocalAppDataRedirectTests` (set/unset/empty routing) + `ENVMANAGER_LOCALAPPDATA` job-private redirect + post-job assertion that real `%LOCALAPPDATA%\\EnvManager` was not created | xUnit + redirect seam + workflow step | Tier 1/3 -- every PR (serial collection) |
| 9 | Randomized state-machine model test | `WritePathStateMachineTests` (CsCheck 4.8.0): 1000 random Rename/ChangeScope/Set/Delete/PathAdd/PathRemove per run; dictionary + broadcast-count model in lockstep; minimal counterexample shrink | xUnit + CsCheck | Tier 1 -- every PR (serial collection) |
| 10 | Differential oracle | `DifferentialOracleTests`: InMemoryScope vs RegistryScope (real registry as oracle) -- REG_EXPAND_SZ preservation, PATH 1024/~30000-char boundaries, empty-entry current-directory semantics, `=`-in-name rejection, elevation-gated system scope, broadcast count agreement | xUnit + `EM_DIFFERENTIAL_ORACLE=1` gate | Tier 3 -- every PR (windows-latest, gated) |
| 11 | Mutation testing | dotnet-stryker 4.16.0 mutating 4 red-line files (rename / change-scope / profile apply-unapply / protection), string/logical mutants ignored, thresholds 85/70/60 - break 60 is the aspirational target line and is currently NOT met (46.83%-49.80%, runs 33966698434/34767318054; score is reported, not CI-gated), local/PR-assist only (v5/dotnet10 pipeline friction) | dotnet-stryker | On-demand (`workflow_dispatch` stryker job) |
| 12 | Mutation survivor triage / kill tests | `MutationSurvivorTriageTests` + `MutationSurvivorTriageStdoutTests`: 13 weak-assertion Stryker baseline survivors killed through existing seams + new `SetAppDataDirectoryForTests` redirect seam; equivalent survivor registered in `reports/18-survivor-registry.json`; round-8 second triage (ticket 50) registers the current survivor set in `reports/50-survivor-registry.md` (kill via seam assertion or LLM-detection reserve) | xUnit | Tier 1 -- every PR (parallel) |
| 13 | CLI output snapshot (golden master) | `CliOutputSnapshotTests` (Verify.Xunit 31.12.5): 17 source-controlled `.verified.txt` (help x2, write-path stdout x2, error copy x12, canary masking x1); `Scrub()` normalizer volatile-field-only + JSON-escape restoration; non-parallel collection for console capture | xUnit + Verify.Xunit | Tier 2 -- every PR (serial collection) |
| 14 | ProfileCommand characterization baseline | `ProfileCommandCharacterizationTests` (Verify.Xunit): 62 snapshots -- launch-injection projection (inheritance-override order, PATH normalize-dedupe, scope preservation, ciphertext-never-plaintext) + per-verb error/usage of all three subdomains (profile-CRUD / set-launch+launch / secret) | xUnit + Verify.Xunit | Tier 2 -- every PR (serial collection) |
| 15 | i18n rendered snapshot | `frontend/src/lib/translations.test.ts`: renders every key of all 10 locales through `intl-messageformat` (svelte-i18n engine); ICU render failure (e.g. `{placeholder}` wrapped in single quotes) turns red | Vitest + svelte-i18n | Tier 1 -- every PR (parallel) |
| 16 | IPC schema contract (single source of truth) | `docs/schemas/env-manager-service-ipc.schema.json` + `ipc-samples.json` exported from `service/src/ipc.rs`; pinned across three clients (C# `ServiceIpcContractTests`, TS `ipc-schema-contract.test.ts`, Tauri `ipc_contract_tests`) | xUnit + Vitest + cargo test | Tier 1/2/4 -- every PR (parallel) |
| 17 | Secret provider contract (shared abstract base) | `SecretProviderContractTests` (abstract base) + 8 sealed mounts (`*ContractTests.cs`): fail-closed decryption, round-trip, stable malformed-format error, plaintext-never-in-envelope -- all through `ISecretProviderHarness` neutral-write/read seam | xUnit + `ISecretProviderHarness` | Tier 2 -- every PR (per-mount serial) |
| 18 | Secret provider compliance gate | `SecretProviderContractComplianceTests`: reflection gate fails the build if an `ISecretProvider` implementation lacks a contract-test subclass | xUnit reflection | Tier 1 -- every PR |
| 19 | Secret provider L0 (real local backend) | `DpapiCurrentUserContractTests`: backend-dependent assertions (round-trip, plaintext-never) on real crypt32 CurrentUser DPAPI | xUnit + real DPAPI | Tier 1 -- every PR |
...
### CI tier definitions (composition, not pyramid)

- **Tier 1 -- xUnit unit + parallel-snapshot**: `dotnet test tests/EnvManager.Engine.Tests/` in the verify job, parallel by default, gated only by serial collections (`CliSnapshotSerial`, `LocalAppDataRedirectSerial`, `DifferentialOracleSerial` when `EM_DIFFERENTIAL_ORACLE=1`).
- **Tier 2 -- snapshot / contract (serial collection)**: Verify.Xunit + reflection compliance + secret-provider contract base; same `dotnet test` invocation, slower because console capture and per-provider state are non-parallel.
- **Tier 3 -- Pester integration / windows-latest only**: `tests/*.Tests.ps1` invoked via `scripts/run-ci-tests.ps1` after CLI binary exists; depends on the staging step from ticket 17 so `tauri.conf.json` `bundle.resources` resolve before Pester steps.
- **Tier 4 -- Rust cargo test**: `cargo test --locked` for `service` and `frontend/src-tauri`; IPC schema + Tauri payload contract + service fault-injection.
- **Tier 1-extra / Tier 4-extra -- on-demand / matrix**: `verify-l1` job (ubuntu-latest, Docker preinstalled) runs `--filter "Category=L1"` with `EM_L1_MATRIX=1`; stryker job (`workflow_dispatch`) runs the four red-line files with per-module score upload.
- **Nightly**: `fuzz.yml` schedule cron `30 18 * * *` runs libFuzzer 1800s; `continue-on-error` on PR short-runs keeps PRs green by design.
- **Manual scripts**: kill -9 administrator script + `EM_TEST_*` self-clean path live outside CI; agents never run them on user machines.

### Composition governance -- adding a new paradigm

Every new test style that wants to land in this repo must (a) name which defect class it catches that the existing 26 do not, (b) name its target module/file, (c) pick a tool already in this list (or justify a new one), and (d) pick the CI tier that matches its risk-vs-cost trade-off. Adding a row requires updating this section and the AGENTS.md Testing paragraph in the same commit; rounds of `reports/NN-*.md` provide evidence. Pluggability rule: any row that fails three PR cycles in a row (red or flake pattern) is demoted from the table with rationale -- preventing paradigm inflation is the explicit purpose of this ledger (round-5 Q4 item 5).
