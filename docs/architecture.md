...
| 23 | Structural-fitness dependency direction / domain isolation (planned, ticket 28) | NetArchTest or equivalent reflection: dependency direction rules (secret domain must not depend on launch/CRUD internals), no cycles, Program methods under cap | NetArchTest (planned) | Tier 1-extra -- every PR (planned) |
