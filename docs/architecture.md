...
| 11 | Mutation testing | dotnet-stryker 4.16.0 mutating 4 red-line files (rename / change-scope / profile apply-unapply / protection), string/logical mutants ignored, thresholds 85/70/60, local/PR-assist only (v5/dotnet10 pipeline friction) | dotnet-stryker | On-demand (`workflow_dispatch` stryker job) |
| 12 | Mutation survivor triage / kill tests | `MutationSurvivorTriageTests` + `MutationSurvivorTriageStdoutTests`: 13 weak-assertion Stryker baseline survivors killed through existing seams + new `SetAppDataDirectoryForTests` redirect seam; equivalent survivor registered in `reports/18-survivor-registry.json` | xUnit | Tier 1 -- every PR (parallel) |
