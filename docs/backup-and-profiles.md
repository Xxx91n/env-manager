...
- Profile names are globally unique across Global and Launch types because CLI commands address profiles by name. This prevents a command from targeting a different profile type than the caller intended.
...
`protection.defaults.json` is source-controlled and embedded into the CLI with a fixed logical resource name. First use seeds editable built-in protection lists under `%LOCALAPPDATA%\EnvManager`. A missing or malformed override falls back to the embedded defaults; updates to this source require tests and documentation in the same change.
